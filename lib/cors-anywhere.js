...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
	...
	// replace query parameter with specified value 
	Object.keys(corsAnywhere.queryParams).forEach( function (key) {
	     req.url = req.url.replace(key + "=0", key + "=" + corsAnywhere.queryParams[key]); 
	}
	);
